...
async function selectTool(toolName) {
  ...
  toolButtons.forEach((button) => {
    button.classList.toggle('primary', button.dataset.adminTool === toolName);
  });

  adminContent.innerHTML = `<div class="muted">Loading...</div>`;
  ...
  else adminContent.innerHTML = `<div class="muted">Unknown tool.</div>`;
}
